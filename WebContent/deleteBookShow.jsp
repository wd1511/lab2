...
<body>
	 ...
	 <section id="intro" class="intro">
		<div class="slogan">
			...
			%>
			图书名称<%=" " %>:<%= newbook.getTitle() %>
			</br>
			出版日期<%=" " %>:<%= newbook.getPublishDate() %>
			</br>
			出版社名<%=" " %>：<%= newbook.getPublisher() %>
			</br>
			作者编号<%=" " %>: <%= newbook.getAuthorID() %>
			</br>
			ISBN编号:<%= newbook.getISBN() %>
			</br>
			图书价格<%=" " %>:	<%= newbook.getPrice() %>
			</br>
			
			<form action = "deletebookshow.action?isbn=<%= newbook.getISBN()%>" method = "post">
